...
<?php 
	...
	if($edit)
	{
		$q = 'UPDATE campaigns SET from_name="'.$from_name.'", from_email="'.$from_email.'", reply_to="'.$reply_to.'", title="'.$subject.'", plain_text="'.$plain.'", html_text="'.addslashes($html).'", bounce_setup = '.$bounce_setup.', complaint_setup = '.$complaint_setup.' WHERE id='.$campaign_id;
	...
	else
	{
		...
		$q = 'INSERT INTO campaigns (userID, app, from_name, from_email, reply_to, title, plain_text, html_text, wysiwyg, bounce_setup, complaint_setup) VALUES ('.get_app_info('main_userID').', '.get_app_info('app').', "'.$from_name.'", "'.$from_email.'", "'.$reply_to.'", "'.$subject.'", "'.$plain.'", "'.addslashes($html).'", '.$wysiwyg.', '.$bounce_setup.', '.$complaint_setup.')';
		$r = mysqli_query($mysqli, $q);
		if ($r)
		{
