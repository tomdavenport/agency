...
<?php 
	...
	$q3 = 'INSERT INTO campaigns (userID, app, from_name, from_email, reply_to, title, plain_text, html_text, bounce_setup, complaint_setup, wysiwyg) VALUES ('.get_app_info('main_userID').', '.$app_id.', "'.$from_name.'", "'.$from_email.'", "'.$reply_to.'", "'.addslashes($title).'", "'.addslashes($plain_text).'", "'.addslashes($html_text).'", '.$bounce_setup.', '.$complaint_setup.', 1)';
