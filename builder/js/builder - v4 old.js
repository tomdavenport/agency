...
editableItems['.pi'] = ['color', 'font-size'];
...
var editableContent = ['.editContent', '.pix_edit', '.navbar a', 'button', 'a.btn', '.footer a:not(.fa)', '.tableWrapper', '.pix_button span'];
...
function makeSortable(el) {
	...
	el.sortable({
		...
		beforeStop: function(event, ui){
			...
			// 		var tt5 = $(this).contents().find( 'body' );
			// 		var fff3 = $(this).contents().find( 'body' ).find(".popup_background");
			// 		 fff3.each(function(i, obj) {
		 ...
		 //     //            tt5.find('#page').append("temp_pop2");
		 //     //            tt5.find('#page').append(temp_pop2);

			// 		 	obj.parentNode.removeChild(obj);
			// 		 	});

					 // var fff4 = $(this).contents().find( 'body' ).find(".popup_wrapper");
					 // fff4.each(function(i, obj) {
					 // 	alert(i);
					 // 	 temp_pop2 = obj.firstChild;
		    //  //            temp_pop2.removeAttribute('style');
		    //  //            temp_pop2.removeAttribute('data-popup-initialized');
		    ...
		    //  //            tt5.find('#page').append(temp_pop2);
					...
					// var fff3 = ui.item.find('iframe').find(".popup_background");
		   //          fff3.each(function(i, obj) {
		   //          	alert(i);
		   //              // temp_pop2 = obj.firstChild;
		   //              // temp_pop2.removeAttribute('style');
...
//=======================================================================================================================
	...
	$('#previewModal').on('shown.bs.modal', function (e) {
		...
		$('#pageList > ul:visible').each(function(){
			...
			//var thepage = $('iframe#skeleton').contents().find('body').html();
			//var asdasd = $thepage.getelementsbyclassname('popup_wrapper');

			var fff = $('iframe#skeleton').contents().find('#page').find(".popup_wrapper");
			fff.each(function(i, obj) {
				temp_pop = obj.firstChild;
			    temp_pop.removeAttribute('style');
			    temp_pop.removeAttribute('data-popup-initialized');
			    temp_pop.removeAttribute('aria-hidden');
			    temp_pop.removeAttribute('role');
			    temp_pop.removeAttribute('tabindex');
			    temp_pop.removeAttribute('aria-labelledby');
			
			    //$('iframe#skeleton').contents().find('#page').append(" === ");
			    $('iframe#skeleton').contents().find('#page').append(temp_pop);
			    //$('iframe#skeleton').contents().find('#page').append(" === ");
			    //alert(temp_pop.html());
			    //$('iframe#skeleton').contents().find('#page').removeChild(obj);
			    obj.parentNode.removeChild(obj);
			...
			var pop_a = $('iframe#skeleton').contents().find('#page').find(".popup_align");
			pop_a.each(function(i, obj) {
				obj.parentNode.removeChild(obj);
			});

			
			
			//$thepage.getelementsbyclassname('popup_wrapper').removeAttribute('style')
			//newInput.val($thepage)
